...
IF OBJECT_ID('dbo.Enterprise', 'U') IS NOT NULL DROP TABLE dbo.Enterprise; -- Drop Enterprise table in case it already exists
IF OBJECT_ID('dbo.ParkEntry', 'U') IS NOT NULL DROP TABLE dbo.ParkEntry; -- Drop ParkEntry table in case it already exists
IF OBJECT_ID('dbo.Time', 'U') IS NOT NULL DROP TABLE dbo."Time"; -- Drop ParkEntry Time in case it already exists
IF OBJECT_ID('dbo.SpeciesType', 'U') IS NOT NULL DROP TABLE dbo.SpeciesType; -- Drop ParkEntry Time in case it already exists
IF OBJECT_ID('dbo.Characteristic', 'U') IS NOT NULL DROP TABLE dbo.Characteristic; -- Drop ParkEntry Time in case it already exists
IF OBJECT_ID('dbo.Period', 'U') IS NOT NULL DROP TABLE dbo."Period"; -- Drop ParkEntry Time in case it already exists
IF OBJECT_ID('dbo.Visit', 'U') IS NOT NULL DROP TABLE dbo.Visit; -- Drop ParkEntry Time in case it already exists
...
CREATE TABLE Enterprise(
	idEnterprise INT IDENTITY(1,1) PRIMARY KEY, -- IDENTITY(1,1) for autoincrement of primary key
...
CREATE TABLE Park(
	...
	fk_idEnterprise INT FOREIGN KEY REFERENCES Enterprise(idEnterprise) NOT NULL,
...
-- Time table creation
CREATE TABLE "Time"(
	idTime INT IDENTITY(1,1) PRIMARY KEY NOT NULL, -- IDENTITY(1,1) for autoincrement of primary key
...
CREATE TABLE Tour(
	...
	Capacity INT NULL,
	fk_idArea INT FOREIGN KEY REFERENCES Area(idArea) NOT NULL,
	fk_idTime INT FOREIGN KEY REFERENCES "Time"(idTime) NOT NULL,
	fk_idVehicle INT FOREIGN KEY REFERENCES Vehicle(idVehicle) NULL
); 
...
CREATE TABLE Employee(
	...
	fk_idAccommodation INT FOREIGN KEY REFERENCES Accommodation(idAccommodation) NULL,
	fk_idTour INT FOREIGN KEY REFERENCES Tour(idTour) NULL,
	Salary INT NOT NULL
);
...
CREATE TABLE AccommodationXTour(
	...
	"Date" DATETIME NULL, -- Double quotes because day is a key word
...
-- Natural park entry table creation.
CREATE TABLE ParkEntry(
	idParkEntry INT IDENTITY(1,1) PRIMARY KEY, -- IDENTITY(1,1) for autoincrement of primary key
	Price INT NOT NULL,
	MantainanceCost INT NOT NULL,
	fk_idPerson INT FOREIGN KEY REFERENCES Person(idPerson) NOT NULL,
	fk_idArea INT FOREIGN KEY REFERENCES Area(idArea) NOT NULL,
	fk_idTime INT FOREIGN KEY REFERENCES "Time"(idTime) NOT NULL,
);